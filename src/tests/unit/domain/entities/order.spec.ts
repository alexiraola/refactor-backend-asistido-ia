...
describe("Order", () => {
  it("should create an order", () => {
    const order = Order.create(Id.create("1"), [OrderItem.create(
      "1", 1, 10
    ).get()], Discount.fromCode("DISCOUNT20").get(), "Nowhere Avenue");
    expect(order).toBeInstanceOf(Order);
  });

  it("should throw if no items are provided", () => {
    expect(() => Order.create(Id.create("1"), [], Discount.fromCode("DISCOUNT20").get(), "Nowhere Avenue")).toThrow();
  });

  it("should calculate the total", () => {
    const order = Order.create(Id.create("1"), [OrderItem.create(
      "1", 1, 10
    ).get()], Discount.fromCode("DISCOUNT").get(), "Nowhere Avenue");
